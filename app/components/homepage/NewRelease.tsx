    ...
    return (
        <>
           <div>
                ...
                <h4 className='text-xs sm:text-xl'>What are your thoughts on this? Share your thoughts with the <span className='text-blue-600' onClick={handleCommunity}>Community</span></h4>
           </div>

           <div className="overflow-x-auto overflow-visible w-full">
                <div className="grid grid-cols-5 xl:grid-cols-6 gap-3 mt-3 min-w-max xl:min-w-full pb-1">
                    {sortedData.map((airing, index) => (
                        <Link key={`${airing}-${index}`} href={`/ADramaUser/${airing.id}`} passHref className='mb-5 xl:mb-0 bg-white shadow-md rounded-lg'>
                            <div className='w-full h-64 xl:h-80 relative rounded-lg'>
                                <Image
                                    fill
                                    loading='lazy'
